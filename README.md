...
**Aiden** is an AI assistant built specifically for **Product Managers**. It helps PMs transform raw product documents — PRDs, customer feedback, research notes, meeting transcripts — into structured deliverables like user stories, roadmaps, metrics plans, and stakeholder updates.
...
| Problem | Aiden's Solution |
|---|---|
| PMs spend hours manually writing PRDs, user stories, and roadmaps | One-click artifact generation from uploaded documents |
| AI chatbots hallucinate or give generic answers | RAG pipeline grounds every answer in the user's actual documents |
| Teams waste time on documents that aren't product-related | Automated relevance guard rejects non-PM content before the AI runs |
...
A two-layer guard (filename check + content analysis) automatically rejects non-product documents like resumes or personal files, keeping the assistant focused on PM work.
...
- **Left Sidebar** — Artifact generation buttons (PRD, User Stories, Roadmap, Metrics, Stakeholder Update) + document upload area with drag-and-drop, file status indicators (uploading → indexing → indexed), and remove buttons
...
│ (Deno)        │ │   (Deno)        │
│               │ │                 │
│ • Chunk text  │ │ • Relevance     │
│ • OpenAI      │ │   guard         │
│   embeddings  │ │ • RAG retrieval │
│ • Store in DB │ │ • Feedback      │
│               │ │   context       │
│               │ │ • AI gateway    │
│               │ │   (streaming)   │
└───────┬──────┘ └────────┬────────┘
...
│   ├── ArtifactMarkdown.tsx   # Rich markdown + email renderer
│   ├── DocumentSidebar.tsx    # File upload + management
│   ├── MessageFeedback.tsx    # Thumbs up/down component
│   └── ui/                    # shadcn/ui components
├── context/
│   └── DocumentContext.tsx    # Document state + embedding orchestration
├── lib/
...
└── functions/
    ├── chat/index.ts          # Chat edge function (RAG + AI)
...
2. Chunks text into ~1500-character segments with 200-char overlap (smart splitting on paragraph/sentence boundaries)
3. Generates vector embeddings via OpenAI `text-embedding-3-small` (batched, with retry logic)
4. Deletes any existing chunks for the same document + session
5. Stores chunks + embeddings in `document_chunks` table

### `chat`
Handles AI conversations:
1. **Relevance Guard** — Checks the latest uploaded document against a classifier (filename cues + content keyword analysis). Rejects non-PM documents immediately.
...
| user_query | text (nullable) | User's original question |
| rating | text | "up" or "down" |
| created_at | timestamptz | Auto-generated |

### `match_document_chunks` (RPC Function)
PostgreSQL function that performs vector similarity search using cosine distance (`<=>` operator) against the pgvector extension. Accepts query embedding, session ID, match count, and similarity threshold.
...
┌─ Chunks found? ──┐
│ Yes               │ No
│ Use as context    │ Fallback: load all chunks
...
- **Reject** only if 2+ strong resume-specific phrases are detected AND no PM keywords exist (e.g., `work experience`, `employment history`, `references available upon request`)
- **Unknown** (pass through to AI) if neither condition is met — erring on the side of acceptance

The guard only runs when the user's message references uploaded documents (keywords like "summarize", "this document", "uploaded", etc.).
...
| **PRD** | Headers + bullet points | Problem, Persona, Goals, Features, Journey, Requirements, Risks, Metrics, Rollout |
...
| **Stakeholder Update** | Email format | Subject line, Progress, Focus, Next Steps, Risks & Blockers |
...
> ⚠️ **No API keys are stored in the codebase.** All secrets are managed via Supabase Edge Function secrets, accessible only at runtime. The `.env` file only contains publishable (non-sensitive) keys.
