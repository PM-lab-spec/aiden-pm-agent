...
function detectArtifactType(content: string): ArtifactType {
  ...
  const lines = lower.split("\n").slice(0, 8); // Only check first 8 lines for detection
  const header = lines.join(" ");

  // Stakeholder update: must have "stakeholder update" in title/header area
  if (
  ...
  // Roadmap: needs Now + Next or Now + Later sections
  if (
    (lower.includes("## now") || lower.includes("### now")) &&
    (lower.includes("## next") || lower.includes("### next") ||
     lower.includes("## later") || lower.includes("### later"))
  ) {
    return "roadmap";
  }
...
const richComponents: Components = {
  ...
  ol: ({ children, ...props }) => (
    <ol className="my-2 ml-4 space-y-1.5 list-decimal" {...props}>
      {children}
    </ol>
  ),
  li: ({ children, ...props }) => (
  ...
  return (
    <div className="my-3 rounded-xl border border-border overflow-hidden bg-card shadow-sm">
      ...
      <div className="px-5 py-4">
        <ReactMarkdown components={richComponents} rehypePlugins={rehypePlugins}>{cleanContent}</ReactMarkdown>
      </div>
...
export default function ArtifactMarkdown({ content }: { content: string }) {
  ...
  return (
    <ReactMarkdown components={richComponents} rehypePlugins={rehypePlugins}>{content}</ReactMarkdown>
  );
}
